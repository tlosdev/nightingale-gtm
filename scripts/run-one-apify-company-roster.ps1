...
try {
    ...
    # L10 — status-code-based detection. See run-one-apify-call.ps1 for rationale.
    $statusCode = $null
    try { $statusCode = [int]$_.Exception.Response.StatusCode } catch { }
    if ($statusCode -eq 404) {
        $base.status = 'apify_actor_not_found'
        $base.error  = "Actor '$ActorId' not found in your Apify account (HTTP 404). Verify in https://console.apify.com/actors and re-run scripts/setup-secrets.ps1 to update apify_company_roster_actor_id."
    } elseif ($statusCode -eq 429) {
        $retryAfter = $null
        try { $retryAfter = $_.Exception.Response.Headers['Retry-After'] } catch {}
        $base.status = 'apify_rate_limited'
        $base.error  = "Apify rate-limited (HTTP 429). Retry-After: $retryAfter. Likely hit free-tier monthly quota."
    } else {
...
# M3 — top-level field check, NOT full-JSON substring. Company-roster
# datasets contain employee titles + bios where any reference to "restricted"
# or "authwall" in a person's headline / company name would false-positive
# and break ALL subsequent intro-finder + daily-brief Apify calls until
# setup-secrets is re-run.
$flagged = $false
...
if ($flagged) {
    # M4 — Set-Content -Value '' instead of New-Item -Force, same rationale
    # as run-one-apify-call.ps1. Defense-in-depth against accidental file
    # truncation if the sentinel path ever collides with something real.
    try {
