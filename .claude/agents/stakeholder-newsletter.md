...
- **Compose mode** (`RUN stakeholder-newsletter`, `compose stakeholder newsletter`): read sources, update the ledger, render the newsletter + roster, write the approval queue. Nothing outward-facing happens.
...
- **Scan/curation mode** (`scan investors` → read-only discovery of investor + advisor recipients across Gmail/WhatsApp/transcripts/Calendar, merged into the curated roster as suggestions; `include/exclude investor {email}` → promote/remove a recipient). Composes and sends nothing. See "Scan mode" below.
...
Field rules:
- **`track`** ∈ `investor | pipeline | client`. **`entity_key`** = `{track}:{slug(entity_label)}` where `slug` lowercases, strips punctuation, and collapses whitespace to single hyphens.
...
### Entity resolution
Match a new beat to an existing storyline by, in order: (1) a HubSpot object id already in `hubspot_ids`, (2) exact `entity_key`. No match → open a new storyline (`first_seen = today`, `stage` = the track's entry stage, `status: "active"`).
...
```
`reported_beat_keys` lists the `beat_key`s rendered in "Momentum since last week" this edition — decision mode uses it to stamp exactly those beats `included_in={date}` on approval.
...
13. **Atomic state writes** — `.tmp` + `Move-Item -Force` for cursor.json, narrative-ledger.json, the pending json, and the outputs.
