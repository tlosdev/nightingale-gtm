...
**Campaign edition (`CAMPAIGN_EDITION` set) — build the edition from the brief.** The body is the campaign storylines materialized in Step 5c for THIS edition, written in the investor register below. **Still lead with the TL;DR (with its raise-status bullet) and include the Core business metrics block** (below) — every campaign edition carries the real numbers (pipeline / leads / customers / revenue) alongside the brief's storylines, from HubSpot (Step 2, read-only) and the ledger. Omit any roll-call/challenge section the ledger doesn't support this edition. Do **NOT** render "Momentum since last week" for the organic beats banked in Steps 2–6 — those stay UNREPORTED (`included_in == null`) and surface in the first organic edition. You may still show the three roll-call sections (Fundraising / Pipeline / Active clients) and the Challenges section as a light status board where the ledger supports them, but the **spine is the brief**. `reported_beat_keys` = the Step 5c campaign beats rendered this edition, and nothing else. (Everything below about register, structure, sensitivity, and rendering still applies.)
...
- **TL;DR (first thing in the body of EVERY edition — campaign and organic):** 3–5 one-line bullets an investor can read in five seconds — the headline stage advance, the core business metrics (below, condensed to one line), and the single most important ask. **Always include exactly one bullet giving the raise status** (e.g. "Raise: $1.2M of $2M committed" or "Raise: no change this week") — the TL;DR is the ONLY place the raise appears; there is no separate raise section. It summarizes what follows and **is not followed by a separate opening/summary paragraph** — go straight into the content. It never introduces anything not covered below.
- **Core business metrics:** a compact block — **pipeline** (open deal value + count), **active leads/prospects**, **customer count** (active clients), and **revenue** (MRR/ARR where known) — sourced from HubSpot (Step 2, read-only) and the ledger's client/pipeline tracks. **Required in every campaign edition;** in organic editions include it whenever the numbers are meaningful. Treat dollar figures as `sensitive` (flag for operator review) and report only what the operator has cleared.
...
- **Fundraising / Pipeline / Active clients:** up to three short sections, each listing that track's `active` storylines with current stage (a one-line status roll-call, so investors see the whole board). **Omit any of the three that has no `active` storylines** — e.g. a first newsletter with no clients yet has no Active clients section. Pipeline + client lines are **traction shown to investors**, not updates to those customers.
...
- **What's next / the ask:** upcoming milestones and the single most important ask (only what the operator has cleared). *(Raise status is not a section here — it is the mandatory TL;DR bullet above.)*
- **Sign-off.**

**Omit every empty section.** A section appears only if it has real content THIS edition. A track with no new momentum, no active clients (e.g. the first newsletter), no challenges, or no upcoming milestone is left out entirely — never render an empty section or a "nothing to report" placeholder. The TL;DR (with its raise bullet) is the only always-present block.
...
### `test stakeholder-newsletter from {date} to {emails}`
A dress rehearsal of the real send to operator-supplied addresses, so the operator can see exactly how an edition lands **before** approving. **Same mechanism as approve — one BCC Gmail draft — but to test recipients and completely non-mutating:**
1. Load `pending/{date}.json` (live or archive). Parse `{emails}` from the trigger: a comma-separated list of **verbatim** operator-entered addresses (provided explicitly for testing, so the never-pattern-guess-emails rule is satisfied). If none are present, report and stop — no draft.
...
2. Each compose run while `status=="active"` and `editions_sent < total_editions` is **campaign edition `editions_sent + 1`**: Step 5c materializes that edition's slice of the brief into ledger storylines; Step 8 renders the edition from those storylines, **led by a TL;DR (with its raise-status bullet) and carrying the Core business metrics block** (pipeline / leads / customers / revenue) so every scripted edition pairs the brief's narrative with the real numbers. Steps 2–6 still ingest organic signals, **banked unreported**.
...
- `test stakeholder-newsletter from {date} to {emails}` — decision mode (dashboard "Send test draft"). Creates a non-mutating `[TEST]` BCC draft to the operator-entered addresses (comma-separated, verbatim); approves and advances nothing.
