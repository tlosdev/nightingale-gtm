...
Nine agents for Nightingale's GTM motion, plus four Windows PowerShell scripts that register most of them with Windows Task Scheduler and capture the credentials the intro-finder + daily-brief Layer-B stages need. The repo is **Windows-only** as of 2026-05 — `.sh` parity was dropped to reduce maintenance surface.

The chain has seven stages (daily-brief Mon-Fri 6am; sweeps + buying-group chained Mon 7am; intro-finder Sun-Fri 7am; gmail-resurfacer Mon-Fri 7am; feedback-analyzer on-demand or weekly; hubspot-manager nightly Mon-Sun 11pm):
...
- **`scripts/start-ui.ps1` + `ui/`** — OPTIONAL local Node.js + React control panel. Loopback-only Express server on `http://localhost:8765`. Renders agent Desktop outputs (brief, signals, intros, re-surfacer, refinements), exposes the hubspot-manager pending-approval queue with inline Apply/Reject buttons, lets the operator trigger any agent on demand. Requires Node 18+ (one-time `npm install`). The chain works identically whether or not the UI is launched. See `ui/README.md`.
...
- **Layer-B Apify Actor ID lives in secrets.json schema v3 as an optional field.** If `apify_company_roster_actor_id` is absent, daily-brief Layer-B falls back to WebSearch automatically. The Layer-B worker script is `scripts/run-one-apify-company-roster.ps1`.
